...
Base Type System
----------------
Since we think that `Arrow`_ is a solid, future-proof choice for in-memory dataframes will hopefully offer a sane
alternative for many `Pandas`_ use cases, we opt for `Arrow`_ as our type system baseline. In contrast to `NumPy`_ and `Pandas`_, it
offers better support for nested types, is more strict about type conversions and can also be used when interacting with
other languages like `R`_, `Julia`_ and `Java`_.

.. note::

   Even though we have chosen `Arrow`_ as our type system, we will demonstrate many things using `NumPy`_ since it
   enables us to easily play around with fixed-size scalar values.
...
Arrow knows a single type here, ``bool``, which does not require any normalization on its own.
...
.. important::
   ...
   `NaN payloads`_ are not handled and should not be used. The `IEEE 754`_ declares them as optinal and hardware and
...
The decimal type ``decimal128[P, S]`` (``P`` denoting the precision and ``S`` the scale) is not touched, since the
exact precision and scale has a semantic meaning.
...
Timestamp
~~~~~~~~~
A combination of `Date`_ and `Time`_ and is particularly useful to store when an event occurred without the need to store
...
``"ms"`` for milliseconds, ``"us"`` for microseconds, ``"ns"`` for nanoseconds; and ``Z`` stands for the timezone). It occupies 64bits.
...
Lists in arrow have a homogeneous element type. We can therefore assume that they can be optimized for certain
...
<https://pandas.pydata.org/pandas-docs/stable/user_guide/gotchas.html#gotchas-intna>`_. Since a ``float64`` only has 53 fraction bits, it cannot store all 64 bit integers:
...
python objects. ``None`` acts as a placeholder "missing value". Arrow requires that values in a columns have one single
type and therefore needs to guess what an ``object`` column should represent (i.e. type inference). If pyarrow does not
find any non-Null object, it treats the column as ``null``. Sadly, this might be wrong. It could easily also have meant
to be a ``string`` or ``date32`` column, but pyarrow cannot know that.
...
|                  |                                    | | ``norm(int64) = int64``                                 |
+------------------+------------------------------------+-----------------------------------------------------------+
| unsigned integer | ``uint{8, 16, 32, 64} -> uint64``  | | ``norm(int8) = int64``                                  |
|                  |                                    | | ``norm(int64) = int64``                                 |
+------------------+------------------------------------+-----------------------------------------------------------+
...
- `Pandas`_:
  ...
  - `Extension Types <https://pandas.pydata.org/pandas-docs/stable/development/extending.html#extension-types>`_
